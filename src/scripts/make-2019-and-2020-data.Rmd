...
private_2019_editorial_board_database <- "ADB40 2019 papers.xlsx"
private_2020_editorial_board_database <- "ADB40 2020 papers.xlsx"

private_2019_committee_decisions_file_name <- "adb40-annual-meeting-2019-decisions"
private_2020_committee_decisions_file_name <- "adb40-annual-meeting-2020-decisions"

local_2019_editorial_file_name <- paste0(confidential_dir, private_2019_editorial_board_database)
local_2020_editorial_file_name <- paste0(confidential_dir, private_2020_editorial_board_database)

local_2019_committee_file_name <- paste0(confidential_dir, "adb40-annual-meeting-2019-decisions.csv")
local_2020_committee_file_name <- paste0(confidential_dir, "adb40-annual-meeting-2020-decisions.csv")
...
https://github.com/tidyverse/googledrive/issues/299
Problem with googledrive methods
...
input_2020_reviews_df <- read_excel(path = local_2020_editorial_file_name, 
                               col_names = LETTERS[1:12])

input_2019_reviews_df <- read_excel(path = local_2019_editorial_file_name, 
                               col_names = LETTERS[1:12])
...
committee_decisions_df <- bind_rows(read_csv(local_2019_committee_file_name, col_types = "ccc"),
                                    read_csv(local_2020_committee_file_name, col_types = "ccc"))
...
# Experiment with ORCID
```{r test}
# rorcid::orcid_auth()

# test_df <- select(canonical_human_df, canonical_name, orcid_url) %>%
#   mutate(orcid_id = str_sub(orcid_url, start = -19L, end = -1L)) %>%
#   filter(!is.na(orcid_id)) %>%
#   head(., n = -20)
# 
# first <- TRUE
# for (subject_orc_id in test_df$orcid_id) {
#   
#   works_df <- rorcid::works(x = subject_orc_id)
#     
#   if (first) {
#     running_df <- works_df
#   }
#   else {
#     running_df <- bind_rows(running_df, works_df)
#     first <- FALSE
#   } 
#   
# }
#   
# count_works_df <- running_df %>%
#   mutate(orcid_extract = str_sub(path, start = 2L, end = 20L)) %>%
#   group_by(orcid_extract) %>%
#   summarise(works = n()) %>%
#   ungroup()
```
...
output_paper_df <- committee_decisions_df %>%
  ...
  left_join(distinct(reviews_df, paper_id, year, title, pub_dec = editorial_status),
            .,
            by = c("paper_id")) %>%
  mutate(prez_dec = toupper(prez_dec)) %>%
  mutate(prez_dec = if_else(prez_dec == "PODIUM", "ACCEPT", prez_dec)) %>%
