...
output_papers_df <- input_papers_df %>%
  ...
  left_join(., matched_humans_papers_df, by = c("author")) %>%
  group_by(paper_id) %>%
...
output_humans_df <- canonical_human_df %>%
  ...
  left_join(., select(matched_humans_papers_df, canonical_name, author_id), by = c("canonical_name")) %>%
...
# Debug
```{r}
df <- output_reviews_df %>%
  filter(paper_id == "20-03210")
