---
title: "Extract Paper Details from PDFs"
...
input_dir <- paste0(confidential_dir, "trb-papers/")

google_drive_path <- "~/project_chandra_bhat/papers/"

output_file_name <- paste0(confidential_dir, "submitted-paper-author-data")

```

# Fetch from Drive
```{r fetch-from-drive}
a_df <- drive_find(q = "name contains '20-0'", type = "pdf")
b_df <- drive_find(q = "name contains 'TRBAM-21-'", type = "pdf")

file_vector <- bind_rows(a_df, b_df) %>%
  pull(., name)
