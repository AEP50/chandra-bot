...
remove(raw_university_df, raw_article_titles_df, remove_word_vector)
...
create_universe_of_humans <- function(input_first_df, input_last_df, input_university_df, input_size) {
  ...
  return_df <- bind_cols(working_first_df, working_last_df, working_university_df) %>%
    rowwise() %>%
    mutate(hash = hash_method(first, last, affiliation)) %>%
    ungroup() %>%
...
output_humans_df <- humans_df %>%
  mutate(name = paste0(first, " ", last),
         ...
         phd_affiliation = as.character(NA),
         google_scholar_url = as.character(NA),
         google_scholar_citations = as.integer(NA),
         google_scholar_h_index = as.integer(NA),
         google_scholar_i10_index = as.integer(NA)) %>%
  select(name, 
         aliases, 
         hash_id = hash, 
...
write_csv(output_reviews_df, path = output_review_file_name)
write_csv(output_humans_df, path = output_human_file_name)
```
